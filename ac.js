    ...
    const beforeEventsPlayerBreakBlock = Minecraft.world.beforeEvents.playerBreakBlock.subscribe(ev => {
      ...
      if (![
        ...
        if (setting.default.data.antiCheat.nukerB.state) {
          if (Date.now() - player.lastBrokeBlock < 5) {
...
const resetValuable = (player) => {
  ...
  let angle = Math.atan2(
    player.location.z - player.lastAction?.location?.z,
    player.location.x - player.lastAction?.location?.x
  ) * 180 / Math.PI - player.lastAction?.rotation?.y - 90
